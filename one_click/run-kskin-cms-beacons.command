...
for c in "$AQUA/KskinCMS" "$AQUA/KskinCMS_SeleniumPython" "$AQUA/kskincms_seleniumpython"; do
  if [[ -f "$c/Beacons_Module/run_beacons_report.bash" ]]; then CMS="$c"; break; fi
done
if [[ -z "$CMS" ]]; then
  echo "ERROR: Cannot find Beacons_Module/run_beacons_report.bash under $AQUA" >&2
...
REPORT="${REPORT_DIR:-$HOME/kskin-web(cms)-automation/reports}/KS-CMS-BEACONS-001_beacons.html"
chmod +x "$CMS/Beacons_Module/run_beacons_report.bash" 2>/dev/null || true
# Do not exec — always open Chrome after the module runner (pass or fail).
set +e
bash "$CMS/Beacons_Module/run_beacons_report.bash"
STATUS=$?
set +e
