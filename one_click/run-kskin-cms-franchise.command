...
for c in "$AQUA/KskinCMS" "$AQUA/KskinCMS_SeleniumPython" "$AQUA/kskincms_seleniumpython"; do
  if [[ -f "$c/Franchise_Account_Module/run_franchise_acc_report.bash" ]]; then CMS="$c"; break; fi
done
if [[ -z "$CMS" ]]; then
  echo "ERROR: Cannot find Franchise_Account_Module/run_franchise_acc_report.bash under $AQUA" >&2
...
REPORT="${REPORT_DIR:-$HOME/kskin-web(cms)-automation/reports}/KS-CMS-FRANCHISE-001_franchisee_accounts.html"
chmod +x "$CMS/Franchise_Account_Module/run_franchise_acc_report.bash" 2>/dev/null || true
# Do not exec — always open Chrome after the module runner (pass or fail).
set +e
bash "$CMS/Franchise_Account_Module/run_franchise_acc_report.bash"
STATUS=$?
set +e
