...
for c in "$AQUA/KskinCMS" "$AQUA/KskinCMS_SeleniumPython" "$AQUA/kskincms_seleniumpython"; do
  if [[ -f "$c/Regions_Module/run_regions_report.bash" ]]; then CMS="$c"; break; fi
done
if [[ -z "$CMS" ]]; then
  echo "ERROR: Cannot find Regions_Module/run_regions_report.bash under $AQUA" >&2
...
REPORT="${REPORT_DIR:-$HOME/kskin-web(cms)-automation/reports}/KS-CMS-REGIONS-001_regions.html"
chmod +x "$CMS/Regions_Module/run_regions_report.bash" 2>/dev/null || true
# Do not exec — always open Chrome after the module runner (pass or fail).
set +e
bash "$CMS/Regions_Module/run_regions_report.bash"
STATUS=$?
set +e
