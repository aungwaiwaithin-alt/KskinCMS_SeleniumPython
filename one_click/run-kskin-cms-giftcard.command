...
for c in "$AQUA/KskinCMS" "$AQUA/KskinCMS_SeleniumPython" "$AQUA/kskincms_seleniumpython"; do
  if [[ -f "$c/GiftCard_Module/run_gift_card_report.bash" ]]; then CMS="$c"; break; fi
done
if [[ -z "$CMS" ]]; then
  echo "ERROR: Cannot find GiftCard_Module/run_gift_card_report.bash under $AQUA" >&2
...
REPORT="${REPORT_DIR:-$HOME/kskin-web(cms)-automation/reports}/KS-CMS-GIFTCARD-001_gift_card.html"
chmod +x "$CMS/GiftCard_Module/run_gift_card_report.bash" 2>/dev/null || true
# Do not exec — always open Chrome after the module runner (pass or fail).
set +e
bash "$CMS/GiftCard_Module/run_gift_card_report.bash"
STATUS=$?
set +e
