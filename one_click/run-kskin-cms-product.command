...
for c in "$AQUA/KskinCMS" "$AQUA/KskinCMS_SeleniumPython" "$AQUA/kskincms_seleniumpython"; do
  if [[ -f "$c/Product_Module/run_product_report.bash" ]]; then CMS="$c"; break; fi
done
if [[ -z "$CMS" ]]; then
  echo "ERROR: Cannot find Product_Module/run_product_report.bash under $AQUA" >&2
...
REPORT="${REPORT_DIR:-$HOME/kskin-web(cms)-automation/reports}/KS-CMS-PRODUCT-001_products.html"
chmod +x "$CMS/Product_Module/run_product_report.bash" 2>/dev/null || true
# Do not exec — always open Chrome after the module runner (pass or fail).
set +e
bash "$CMS/Product_Module/run_product_report.bash"
STATUS=$?
set +e
