#!/bin/bash
# Android E2E Full Flow — one-click (Claude-style, self-contained)
...
if ! curl -s http://127.0.0.1:4723/status >/dev/null 2>&1; then
  if command -v appium >/dev/null 2>&1; then
    nohup appium --port 4723 >reports/appium_android_e2e.log 2>&1 &
...
if [[ -n "$TEST" ]]; then
  ...
  mapfile -t FILES < <(find tests -type f -name '*.py' \( -iname '*e2e*android*' -o -iname '*android*e2e*' -o -iname '*full_flow*android*' \) ! -name '__init__.py' 2>/dev/null | sort)
  if [[ ${#FILES[@]} -eq 0 ]]; then
    echo "ERROR: no Android E2E tests under tests/"
