#!/bin/bash
# KSKIN_MOBILE_RUNNER_VENV — Android E2E full flow (project .venv, never Homebrew pip).
set -uo pipefail

AQUA="${AQUA_ROOT:-$HOME/AquaProjects}"
APPIUM_PY="${APPIUM_PY:-$AQUA/MCP_Appium_Server/python}"
REPORT_HTML="$APPIUM_PY/reports/KS-E2E-AND-001_full_flow.html"
PKG="${ANDROID_UAT_PACKAGE:-com.kskinfacial.customer.uat}"

echo "KSKIN_MOBILE_RUNNER_VENV=android-e2e"
...
echo "========================================"
echo "  Kskin Android — E2E Full Flow"
echo "  Dir: $APPIUM_PY"
...
ensure_appium "$APPIUM_PY/reports/appium_android_e2e.log"
...
echo "[3/5] Uninstalling $PKG for a clean baseline..."
adb uninstall "$PKG" >/dev/null 2>&1 || echo "  (already clean)"
ensure_dynamic_data

echo "[4/5] Discovering Android E2E pytest targets..."
TARGETS=()
for pref in \
  "tests/test_e2e_full_flow_android.py" \
  "tests/test_e2e_android.py" \
  "tests/test_full_flow_android.py" \
  "tests/e2e/test_android.py" \
  "tests/e2e"
do
  if [[ -e "$pref" ]]; then
    TARGETS=("$pref")
...
if [[ ${#TARGETS[@]} -eq 0 ]]; then
  while IFS= read -r f; do
    TARGETS+=("$f")
  done < <(find tests -type f -name '*.py' \( -iname '*e2e*android*' -o -iname '*android*e2e*' -o -iname '*full_flow*android*' \) ! -name '__init__.py' 2>/dev/null | sort)
fi
if [[ ${#TARGETS[@]} -eq 0 ]]; then
  echo "ERROR: No Android E2E test files found under $APPIUM_PY/tests"
...
if [[ ! -f "$REPORT_HTML" ]]; then
  NEWEST="$(ls -t "$APPIUM_PY"/reports/*E2E*AND*.html \
    "$APPIUM_PY"/reports/*e2e*android*.html \
