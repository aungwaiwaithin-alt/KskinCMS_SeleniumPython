...
for c in "$AQUA/KskinCMS" "$AQUA/KskinCMS_SeleniumPython" "$AQUA/kskincms_seleniumpython"; do
  if [[ -f "$c/GiftBundle_Module/run_gift_bundle_report.bash" ]]; then CMS="$c"; break; fi
done
if [[ -z "$CMS" ]]; then
  echo "ERROR: Cannot find GiftBundle_Module/run_gift_bundle_report.bash under $AQUA" >&2
...
REPORT="${REPORT_DIR:-$HOME/kskin-web(cms)-automation/reports}/KS-CMS-GIFTBUNDLE-001_gift_bundles.html"
chmod +x "$CMS/GiftBundle_Module/run_gift_bundle_report.bash" 2>/dev/null || true
# Do not exec — always open Chrome after the module runner (pass or fail).
set +e
bash "$CMS/GiftBundle_Module/run_gift_bundle_report.bash"
STATUS=$?
set +e
