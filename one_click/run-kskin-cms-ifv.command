...
for c in "$AQUA/KskinCMS" "$AQUA/KskinCMS_SeleniumPython" "$AQUA/kskincms_seleniumpython"; do
  if [[ -f "$c/IssueFreeVoucher_Module/run_ifv_report.bash" ]]; then CMS="$c"; break; fi
done
if [[ -z "$CMS" ]]; then
  echo "ERROR: Cannot find IssueFreeVoucher_Module/run_ifv_report.bash under $AQUA" >&2
...
REPORT="${REPORT_DIR:-$HOME/kskin-web(cms)-automation/reports}/KS-CMS-IFV-001_issue_free_vouchers.html"
chmod +x "$CMS/IssueFreeVoucher_Module/run_ifv_report.bash" 2>/dev/null || true
# Do not exec — always open Chrome after the module runner (pass or fail).
set +e
bash "$CMS/IssueFreeVoucher_Module/run_ifv_report.bash"
STATUS=$?
set +e
