#!/bin/bash
# iOS E2E Full Flow — one-click (Claude-style, self-contained)
...
REPORT="$APPIUM_PY/reports/KS-E2E-iOS-001_full_flow.html"
...
if ! curl -s http://127.0.0.1:4723/status >/dev/null 2>&1; then
  if command -v appium >/dev/null 2>&1; then
    nohup appium --port 4723 >reports/appium_ios_e2e.log 2>&1 &
...
if [[ -n "$TEST" ]]; then
  ...
  mapfile -t FILES < <(find tests -type f -name '*.py' \( -iname '*e2e*ios*' -o -iname '*ios*e2e*' -o -iname '*full_flow*ios*' \) ! -name '__init__.py' 2>/dev/null | sort)
  if [[ ${#FILES[@]} -eq 0 ]]; then
    echo "ERROR: no iOS E2E tests under tests/"
...
[[ -f "$REPORT" ]] || REPORT="$(ls -t reports/*E2E*iOS*.html reports/*e2e*ios*.html reports/*.html 2>/dev/null | head -1 || true)"
