...
def main() -> int:
    ...
    cases = [
        ...
        (
            4,
            "Edit default required validation",
            "Empty Percentage / Minimum Fee / both",
            lambda: fee_edit_default_required_validation(page),
        ),
        (
            5,
            "Edit default % + restore",
            "Save temp then restore",
            lambda: fee_edit_default_restore(page),
        ),
        (
            6,
            "Edit Minimum Fee + restore",
            "Dialog S$ min fee round-trip",
            lambda: fee_edit_minimum_fee_restore(page),
        ),
        (
            7,
            "Outlet override + restore",
            "QA outlet % round-trip",
            lambda: fee_override_outlet_restore(page),
        ),
