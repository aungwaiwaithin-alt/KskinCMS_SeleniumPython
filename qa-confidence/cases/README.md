...
1. Prefer a module batch file (`product_cases.yaml`) or copy `_example_case.yaml` → `MC-<module>-<n>.yaml`.
2. Set `case_id`, `title`, `module`.
3. Set `scenario_id` when automation exists; leave empty for **unmapped**.
4. Regenerate from the **CMS git repo** root: `python3 scripts/generate_qa_confidence_report.py`
